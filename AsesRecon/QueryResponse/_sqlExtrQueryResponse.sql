...
,Resp.AsesBirthDate,Resp.AsesRegionCode,CAST(CAST(Resp.CarrierEffectiveDate AS date)AS datetime)'CarrierEffectiveDate'
,Resp.QuerySource,Resp.AsesCoverageCode,Resp.MemberSuffix,Resp.ODSIFamilyID,Resp.MPI,Resp.MessageCode
...
,Ver.AsesBenefitPackageType,Ver.AsesBenefitPackageVersion,Ver.BenefitPackageMRefId as VerBenefitPackageMRefId 
,Ver.AsesCoverageCode as VerAsesCoverageCode,Curr.NewCarrier,CASE WHEN RESP.CarrierEffectiveDate IS NOT NULL 
...
FROM dbo.vw_QueryResponseMemberCurrent Resp
	JOIN XRef.Carrier XC 
	ON RTRIM(XC.ExternalValue) = RTRIM(Resp.AsesCarrier)
	JOIN MRef.Carrier MC 
	ON MC.CarrierMRefId = XC.CarrierMRefId
	
	JOIN dbo.Member M 
	ON M.CarrierMRefId = XC.CarrierMRefId 
	AND M.MPI = Resp.MPI



	JOIN dbo.MemberHealthcareAdmin HCFA 
	ON HCFA.MemberRecId = M.MemberRecId
	JOIN dbo.MemberBenefitPackage MMBP 
	ON M.MemberRecId = MMBP.MemberRecId
	LEFT JOIN dbo.vw_BenefitPackageVersion Ver 
	ON Ver.BenefitPackageMRefId = MMBP.BenefitPackageMRefId AND CAST(MMBP.EffectiveDate AS date) BETWEEN Ver.AsesVersionEffectiveDate AND ISNULL(CONVERT(date, Ver.AsesVersionExpirationDate), '20781231')  AND Ver.AsesCoverageCode = Resp.AsesCoverageCode  
	LEFT JOIN stg.EligibilityMemberCurrent Curr 
	ON Curr.MPI = Resp.MPI
	LEFT JOIN dbo.MemberEnrollmentStatus MES 
	ON MES.MemberRecId = M.MemberRecId AND Resp.CarrierEffectiveDate IS NOT NULL
	...
	LEFT JOIN STG.ELIGIBILITYMEMBERHISTORY HIS 
	ON HIS.MPI = RESP.MPI AND HIS.OdsiFamilyNumber = Resp.ODSIFamilyID 
...
WHERE 1=1
 AND Resp.CarrierFutureEffectiveDate IS NULL /* Query Response information filter */
...
/*Future pkg*/
AND ((MMBP.EffectiveDate > GETDATE()   
AND (MMBP.TerminationDate IS NULL OR MMBP.TerminationDate >= DATEADD(s, -1, DATEADD(mm, DATEDIFF(m, 0,GETDATE())+ 2, 0))))  
/*Retro pkg*/
OR (MMBP.EffectiveDate <= ISNULL(Resp.CarrierEffectiveDate, '19000101')
AND (MMBP.TerminationDate IS NULL OR CONVERT(DATE, MMBP.TerminationDate) >= CONVERT(DATE, DATEADD(s, -1, DATEADD(mm, DATEDIFF(m, 0,Resp.CarrierEffectiveDate) + 1, 0))))))
